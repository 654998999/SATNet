...
for i in range(epochs):
    ...
    if min_val_loss > mse_val ** 0.5:
        ...
        print("Saving...")
        torch.save(model.state_dict(), "../SATNet/model_pm25.pt")
        counter = 0
    else:
        counter += 1
    ...
    if (i % 10 == 0):
        ...
        plt.plot(preds) #蓝色
        plt.plot(true) #橙色
        plt.savefig(f'picture/train{i}.png')
        plt.show()


model.load_state_dict(torch.load("../SATNet/model_pm25.pt"))
with torch.no_grad():
    mse_val = 0
    preds = []
    true = []
...
plt.plot(preds)
plt.plot(true)
plt.savefig(f'picture/testtest.png')
plt.show()