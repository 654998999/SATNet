...
X_test = X[val_bound:-3] #8765*10*11
y_train = y[3:train_bound+3]
y_val = y[train_bound+3:val_bound+3]
y_test = y[val_bound+3:]
...
for i in range(epochs):
    ...
    if min_val_loss > mse_val ** 0.5:
        ...
        print("Saving...")
        torch.save(model.state_dict(), "../SATNet/model_pm251.pt")
        counter = 0
    else:
        counter += 1
    ...
    if (i % 10 == 0):
        ...
        plt.plot(preds) #蓝色
        plt.plot(true) #橙色
        plt.savefig(f'picture/train1{i}.png')
        plt.show()


model.load_state_dict(torch.load("../SATNet/model_pm251.pt"))
with torch.no_grad():
    mse_val = 0
    preds = []
    true = []
...
plt.plot(preds)
plt.plot(true)
plt.savefig(f'picture/testtest1.png')
plt.show()