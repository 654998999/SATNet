...
data = pd.read_csv("../dataset/PM2.5.csv")
cbwd = pd.get_dummies(data['cbwd'])#表头是cbwd的四个值NE NW SE cv 是某个值为1其余为0
cbwd.columns = ["cbwd_{}".format(i) for i in range(cbwd.shape[1])] #将表头那四个值变为cbwd_0——cbwd_3
data = pd.concat([data, cbwd], axis=1) #将cbwd那四列加到data后边
data.drop(['cbwd'], axis=1, inplace=True) #把本来的cbwd那列去掉
print(data.head())  #返回data的前几行数据，默认为前五行
cols = list(data.columns[5:])
print(cols)
data['pm2.5'] = data['pm2.5'].fillna(method='ffill').fillna(method='bfill') #将NA先用前面最近的填充 如果前面没值就用后面最近的
depth = 48
...
X_test = X[val_bound:-6] #8765*10*11
y_train = y[6:train_bound+6]
y_val = y[train_bound+6:val_bound+6]
y_test = y[val_bound+6:]
...
epoch_scheduler = torch.optim.lr_scheduler.StepLR(opt, 20, gamma=0.7)
...
for i in range(epochs):
    ...
    if min_val_loss > mse_val ** 0.5:
        ...
        print("Saving...")
        torch.save(model.state_dict(), "../SATNet/model_pm252.pt")
        counter = 0
    else:
        counter += 1
    ...
    if (i % 10 == 0):
        ...
        plt.plot(preds) #蓝色
        plt.plot(true) #橙色
        plt.savefig(f'picture/train2{i}.png')
        plt.show()


model.load_state_dict(torch.load("../SATNet/model_pm252.pt"))
with torch.no_grad():
    mse_val = 0
    preds = []
    true = []
...
plt.plot(preds)
plt.plot(true)
plt.savefig(f'picture/testtest2.png')
plt.show()